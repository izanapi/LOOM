import { frequency, clamp } from './music.mjs?v=tonic-15';
import { pluckedWave } from './voices.mjs?v=tonic-15';
import { WEFTS, isHarmony, weftRelease } from './loom.mjs?v=tonic-15';
  ...
  start(notes,row,{when=this.audio.time,level=.7,duration=Infinity,pan=0,source='live',id=0,attack=.85,voice='qanun',retrigger=false}={}) {
    ...
    if(kind!=='echo'&&!sharedVoice) notes.forEach((note,i)=>{
      ...
      if(grain) {
        ...
        osc.detune.value=kind==='drone'?0:(i-1)*2.2;
      }
