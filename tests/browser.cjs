...
(async()=>{
 ...
 const point=(id,row)=>({x:box.x+left+id*(right-left)/13,y:box.y+rowTop+row*(rowBottom-rowTop)/12});
 ...
 await cdp.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[{x:lowerX,y:point(0,12).y,id:4}]});
 await cdp.send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});
 ...
 await page.mouse.move(bassX(0),point(0,12).y);await page.mouse.down();await page.waitForTimeout(550);
 assert.match(await page.locator('#noteReadout').textContent(),/^C1 × DRONE/);
 await page.mouse.move(box.x+left-2,point(0,12).y,{steps:5});await page.waitForTimeout(150);
 assert.match(await page.locator('#noteReadout').textContent(),/^C1 × DRONE/);
 await page.mouse.move(bassX(7),point(0,12).y,{steps:14});await page.waitForTimeout(250);
 assert.match(await page.locator('#noteReadout').textContent(),/^C1 × DRONE/);
