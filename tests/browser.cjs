...
(async()=>{
 ...
 assert.ok(await page.locator('script[type="module"]').getAttribute('src').then(s=>s.includes('?v=restrike-14')));
 const geometry=await page.evaluate(async()=>{const m=await import('./loom.mjs?v=restrike-14');return m.loomGeometry(390,500)});
