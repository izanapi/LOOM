...
(async()=>{
 ...
 assert.ok(await page.locator('script[type="module"]').getAttribute('src').then(s=>s.includes('?v=corner-3')));
 const geometry=await page.evaluate(async()=>{const m=await import('./loom.mjs?v=corner-3');return m.loomGeometry(390,500)});
