...
(async()=>{
 ...
 assert.ok(await page.locator('script[type="module"]').getAttribute('src').then(s=>s.includes('?v=drone-left-13')));
 const geometry=await page.evaluate(async()=>{const m=await import('./loom.mjs?v=drone-left-13');return m.loomGeometry(390,500)});
